...
class InferenceProcessor:
    ...
    def __init__(self, cfg: InferConfig) -> None:
        ...
        self.generator = Decoder(cfg.decoding, self.tgt_dict) # viterbi
    ...
    def process_sample(self, sample: Dict[str, Any]) -> None:
        ...
        hypos, encoder_out = self.task.inference_step(
...
def main(cfg: InferConfig) -> float:
    ...
    with InferenceProcessor(cfg) as processor:
        for sample in processor:
            encoder_out = processor.process_sample(sample)
        ...
        def plot_tsne(encoder_outs):
            # Convert encoder_outs to numpy array
            encoder_outs = np.array(encoder_outs)

            # Apply t-SNE dimensionality reduction
            tsne = TSNE(n_components=2, random_state=42)
            embeddings = tsne.fit_transform(encoder_outs)
        ...
        # Call the function to plot t-SNE
        # plot_tsne(encoder_outs)
        return wer
