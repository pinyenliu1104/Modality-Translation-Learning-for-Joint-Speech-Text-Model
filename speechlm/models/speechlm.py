...
class SpeechlmConfig(HubertConfig):
    load_checkpoint: Optional[str] = field(
        default=None,
        metadata={"help": "load checkpoint for the model"},
    )
...
class SpeechlmModel(BaseFairseqModel):
    ...
    def build_model(cls, cfg: SpeechlmConfig, task: HubertPretrainingTask):
        ...
        if cfg.load_checkpoint:
            state = model.load_checkpoint(cfg.load_checkpoint)
            model.load_state_dict(state["model"], strict=True)
            logger.info(f"Loaded model from {cfg.load_checkpoint}")
            logger.info(f"Loaded model state: {state.keys()}")
        return model
    ...
    def forward(
        ...
        if src_tokens is None:
            ...
            return self.forward_speech_text(
                source=source,
                src_tokens=src_tokens,
                src_lengths=src_lengths,
                target_list=target_list,
    ...
    def forward_speech_text(
        self,
        # speech input
        speech_source: torch.Tensor = None,
        speech_target_list: Optional[List[torch.Tensor]] = None,
        speech_padding_mask: Optional[torch.Tensor] = None,
        speech_mask: bool = True,
        speech_features_only: bool = False,
        speech_output_layer: Optional[int] = None,
        # text input
        text_src_tokens: torch.Tensor = None,
        text_src_lengths: torch.Tensor = None,
        text_target_list: Optional[List[torch.Tensor]] = None,
        text_mask: bool = True,
        text_output_layer: Optional[int] = None,
    ) -> Dict[str, torch.Tensor]:
        # forward speech through the speech encoder
        speech_features = self.forward_features(speech_source)
        if speech_target_list is not None:
            speech_features, speech_target_list = self.forward_targets(speech_features, speech_target_list)

        speech_features_pen = speech_features.float().pow(2).mean()

        speech_features = speech_features.transpose(1, 2)
        speech_features = self.layer_norm(speech_features)
        unmasked_speech_features = speech_features.clone()

        if speech_padding_mask is not None:
            speech_padding_mask = self.forward_padding_mask(speech_features, speech_padding_mask)

        if self.post_extract_proj is not None:
            speech_features = self.post_extract_proj(speech_features)

        speech_features = self.dropout_input(speech_features)
        unmasked_speech_features = self.dropout_features(unmasked_speech_features)

        if speech_mask:
            speech_x, speech_mask_indices = self.apply_mask(speech_features, speech_padding_mask, speech_target_list)
        else:
            speech_x = speech_features
            speech_mask_indices = None

        # speech_feature: (B, T, D), float
        # speech_target: (B, T), long
        # speech_x: (B, T, D), float
        # speech_padding_mask: (B, T), bool
        # speech_mask_indices: (B, T), bool
        speech_x, _ = self.encoder(
            speech_x,
            padding_mask=speech_padding_mask,
            layer=None if speech_output_layer is None else speech_output_layer - 1,
        )

        if speech_features_only:
            return {"x": speech_x, "padding_mask": speech_padding_mask, "features": speech_features}
        
        # concatenate speech and text to compute the final logits
        if self.add_unit_encoder:
            # get speech embeddings
            speech_src_tokens, speech_x_emb, l2_loss = self.convert_embeddings(
                speech_x, 
                speech_padding_mask, speech_target_list[0],
                mask_indices=speech_mask_indices,
                mix_with_unit=self.mix_with_unit,
                use_pred_unit=self.use_pred_unit,
                l2_embedding=self.l2_embedding,
            )
            # get text embeddings
            text_padding_mask = text_src_tokens == self.padding_idx
            text_unit_embeddings = self.unit_embed_tokens(text_src_tokens)
            if text_mask:
                text_unit_embeddings, text_mask_indices = self.apply_mask(text_unit_embeddings, text_padding_mask, [text_src_tokens])
            else:
                text_mask_indices = text_target_list[-1] != self.padding_idx
                text_unit_embeddings[text_mask_indices] = self.mask_emb

            # concatenate speech and text embeddings
            src_tokens = torch.cat([speech_src_tokens, text_src_tokens], dim=1)
            x_emb = torch.cat([speech_x_emb, text_unit_embeddings], dim=1)
            padding_mask = torch.cat([speech_padding_mask, text_padding_mask], dim=1)
            mask_indices = torch.cat([speech_mask_indices, text_mask_indices], dim=1)

            # forward the concatenated embeddings through the unit encoder
            encoder_out = self.unit_encoder(src_tokens, token_embeddings=x_emb)
            ...
            result = {
                ...
                "logit_u_list": logit_u_list,
                "padding_mask": padding_mask,
                "features_pen": speech_features_pen,
            }
            result['encoder_out'] = encoder_out['encoder_out']
            result['encoder_padding_mask'] = encoder_out['encoder_padding_mask']
            if self.l2_embedding:
                result['embedding_l2_loss'] = l2_loss
        return result
